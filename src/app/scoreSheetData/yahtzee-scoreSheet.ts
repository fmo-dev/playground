...
const bonusRequired = 63;
const bonusPoints = 35;
const fullHousePoints = 25;
const suiteIndic = 10;
const yahtzeePoints = 50;

const initNumberScore = (nb) => {
    for (let i = 1; i <= nb; i++) {
        YahtzeeScoreSheet.push(getNumberScoreLine(i));
    }
};


const initOfAKindScore = () => {
    for (let i = 3; i <= 4; i++) {
        YahtzeeScoreSheet.push(getOfAKindScore(i));
    }
};
const initSuiteScore = () => {
    for (let i = 4; i <= 5; i++) {
        YahtzeeScoreSheet.push(getSuiteScoreLine(i));
    }
};

const getNumberScoreLine = (i) => {
    return { value: NumberScoreLineTitle[i - 1], calcFunction: calcNumber, option: i, clickable: true, total: false };
}
...
const getSuiteScoreLine = (i) => {
    return { value: (i < 5 ? 'Low' : 'High') + ' Straight', calcFunction: calcSuite, option: i, clickable: true, total: false };
}

const getYahtzeeScore = () => YahtzeeScoreSheet.push({ value: 'Yahtzee', calcFunction: calcYahtzee, clickable: true, total: false });

const getChanceScore = () => YahtzeeScoreSheet.push({ value: 'Chance', calcFunction: calcTotalDices, clickable: true, total: false });
...
const calcSameNumber = (params: any) => {
    for (let i = 0; i <= params.dices.length - params.option; i++) {
        ...
        for (let j = 0; j < params.dices.length; j++) {
            if (j != i && params.dices[i] == params.dices[j]) {
                sameNumber++;
            }
        }
        if (sameNumber >= params.option) {
            return calcTotalDices(params);
        }
    }
    return 0;
}
...
const calcTotalDices = (params: any) => {
    let result: number = 0;
    params.dices.forEach(dice => result += dice);
    return result;
}
...
const calcTotalNumberScore = (sheet: ScoreSheet) => {
    ...
    for (const line in sheet) {
        if (NumberScoreLineTitle.includes(line)) {
            result += sheet[line];
        }
    }
    return result;
...
const calcTotalTwo = (sheet: ScoreSheet) => {
    ...
    for (const line of YahtzeeScoreSheet) {
        if (!NumberScoreLineTitle.includes(line.value) && line.clickable) {
            result += sheet[line.value];
        }
    }
    return result;
}
...
const YahtzeeScoreSheet: ScoreSheet[] = [];
initNumberScore(6);
totalNumberScore();
bonusLine();
totalOne();
